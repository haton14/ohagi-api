...
// The RecordFoodFunc type is an adapter to allow the use of ordinary
// function as RecordFood mutator.
type RecordFoodFunc func(context.Context, *ent.RecordFoodMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f RecordFoodFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.RecordFoodMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.RecordFoodMutation", m)
	}
	return f(ctx, mv)
}
